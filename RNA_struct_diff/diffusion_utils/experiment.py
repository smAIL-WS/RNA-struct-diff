...
from diffusion_utils.base import DataParallelDistribution, DDPDistribution
...
class DiffusionExperiment(BaseExperiment):
    ...
    def __init__(self, args,
        ...
        elif getattr(args, 'ddp', False):  # DDP support
            from torch.nn.parallel import DistributedDataParallel as DDP
            model = DDPDistribution(model, device_ids=[args.local_rank], output_device=args.local_rank)
        ...
        if args.log_tb and is_main_process():
    ...
    def log_fn(self, epoch, train_dict, eval_dict):
        ...
        # Tensorboard
        if self.args.log_tb and is_main_process():
        ...
        # Weights & Biases
        if self.args.log_wandb and is_main_process():
            for metric_name, metric_value in train_dict.items():
